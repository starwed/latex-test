...
The fermion fields are $\Psi$, the photon field is $A$.  $p$ is the momentum operator, $m$ the particle's mass, and $e$ the electron's charge.  The $\gamma$ matrices mix the different components.  %TODO better explanation of gamma matrices
...
\beq	
	q \cdot (2p + q) = (p' - p) \cdot (p' + p) = p'^2 - p^2 =0 \to q^2 = -\frac{1}{2} p \cdot q
\eeq
