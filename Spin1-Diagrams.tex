\chapter{Spin one: Derivation of nonrelativistic Hamiltonian --- the diagrammatic approach}
...
\beq
\begin{split}
\mathcal{L}_A = \Psi^\dagger \Bigg(&  -eA_0- ie  \frac{ \{\nabla_i, A_i \} }{2m} -ie \frac{ \{\grad^2, \{\nabla_i, A_i \}  \} }{8m^3} 
		+ c_F e \frac{\v{S} \smalldot \v{B}} {2m}   	
...
\small
\beqaL \label{eq:S1:nrqedScatter}
	iM &=&
...
Together with gauge invariance the above process is enough to fix all relevant coefficients, but for consistency the two photon process of Compton scattering can also be considered in NRQED.
...
Applying this to $M_q$, the result is:
 \beq
\begin{split}
\Mq =  &\,	-ie  \phi^\dagger(p') \Big( A_0  + \frac{A_0}{2m^2}( \v{q}^2 + i \v{S} \cdot \v{p} \times \v{q} - (\v{S} \cdot \v{q})^2 )
	\\&	- \frac{(\v{p} + \v{p'})\cdot \v{A} }{2p_0}
	 - \frac{(\v{p} + \v{p'})\cdot \v{A} }{2m} \frac{i \v{S} \cdot \v{p} \times \v{q}}{2m^2} \Big ) \phi(p).
\end{split}
\eeq 
...
Before calculating the specific term in question, consider the general type of term that looks like $\omout{\nu} \omin{\mu} u^\nu v^\mu$.  This should be expressed as a matrix element between the vector part of the polarization $\W$. 
\beqa
\omout{\nu} \omin{\mu} u^\nu v^\mu
	&=&	\W_0'  u_0 v_0 \W_0
...
\begin{minipage}{1.2in}
  \begin{center} \includegraphics[scale=0.7]{eps/blob1} \end{center} 
\end{minipage}
$= \hspace{1em}$
\begin{minipage}{1.2in}
  \begin{center} \includegraphics[scale=0.6]{eps/two-photon-fundamental} \end{center} 
\end{minipage}
$\hspace{1em}+$
\begin{minipage}{1.2in}
   \begin{center} \includegraphics[scale=1]{eps/crossed-small} \end{center} 
\end{minipage}
$+$
\begin{minipage}{1.2in}
   \begin{center} \includegraphics[scale=1]{eps/uncrossed-small} \end{center} 
\end{minipage}
...
\begin{center}\VertexEq{two-photon-fundamental}{
...
The idea is that the two parts of the propagator with poles represent two types of processes, one involving intermediate particles, the other intermediate anti-particles.  These particles are to be considered on-shell, so it is useful to define Lorentz four vectors which represent the momentum of on-shell particles and anti-particles.  For the particles, let $\qp = (\sqrt{\v{q}^2+m^2}, \v{q})$.   And for the antiparticles, let $\qm = (-\sqrt{\v{q}^2+m^2}, \v{q})$.  Also define $E_q = \sqrt{\v{q}^2+m^2}$.
...
Here $q$ is not on mass-shell.  This will be written as the sum of two pole terms, having as their numerator a sum over intermediate on mass-shell states.  In comparison to the spin one-half case, there will be an additional constant which for now can be called $x^{\mu\nu}$.
\beq
...
So the different components of $P^{\mu\nu}$ are, to the order needed,
\beq \label{eq:S1:Pvalues}
 \begin{split} 
 P^{00} =&\, \frac{i}{m^2}, 	 \\
...
There are two tree diagrams, crossed and uncrossed.  Each consists of a propagator and two vertices contracted with external fields.  In the previous section the propagator was considered; the result is that it may simply be replaced by the above quantities $P^{\mu\nu}$ to compare with the local vertices of NRQED.    Now the part of the calculation involving the vertices is considered.
...
\eeq
%Term 3
And in the third term $ - \A \cdot \W (p-[g-1]k)^i$, the $\A_0$ term is of too high order (regardless of the gauge), leaving
\beq
...
So of course parallel calculations for the other vertices could be done, as previously mentioned it is easier to note that they have the same general form.  To obtain $\overline{V}$ the recipe is that $p \to p'$, $k \to -k'$, $\W \to \W^\dagger$.  Similar considerations work for $U$, where the only difference is that because of the crossed lines, $k$ and $k'$ are swapped.
...
If we write the Hamiltonian instead (using $\v{D} = i (\v{p} - e \v{A})$)

\small
\beqa
	\mathcal{H}_{NRQED} &=& \Psi^\dagger \Big \{  \frac{(\v{p}-e\v{A})^2}{2m} - \frac{(\v{p}-e\v{A})^4}{8m^3}
		 + eA_0 
		- e g  \frac{\v{S} \smalldot \v{B}} {2m}   	
		- e(g-1)\frac{ ( \partial_i E_i ) }{6m^2}	
	\\&&	+ e(g-1) \frac{ Q_{ij} ( \partial_i E_j ) }{2m^2}	
	...
	\\&&	+ e\frac{   (\v{S} \smalldot \v{B} ) \v{p}^2  }{2m^3}	
		+(g-2) \frac{ e  (\v{S} \smalldot \v{p}) (\v{B} \smalldot \v{p})  }{4m^3} \Big \} \Psi.
\eeqa
\normalsize
