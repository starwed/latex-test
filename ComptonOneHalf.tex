...
The intermediate positron states will be labelled $\chi$ (and $\chi^\dagger$), and the incoming/outgoing electron states $\phi$ (and $\phi^\dagger$).  The incoming momentum is $p$, the outgoing $p'$, and the intermediate momentum (possessed by the anti-particle) will be called $q$.
...
The interaction is a combination of the vector and tensor terms: $\Gamma^\mu(k) = F_1 \gamma^\mu + F_2 \frac{i k_\nu \sigma^{\mu\nu} }{2m}$.  These terms must be expressed nonrelativistically, including terms of up to one power of the momentum.  Higher order terms may be ignored.  So the bispinors themselves are needed only in the first approximation.
...
The tensor bilinear will always appear coupled with the photon momentum.  So only terms of order unity are needed.
First consider the part of the tensor bilinear with one spatial and one time-like index:
\beqa
	\ubar(p') \sigma^{0i} v(q)
		&=& i	\udaggervec{p'} \begin{pmatrix} 0 & \sigma_i \\ -\sigma_i & 0  \end{pmatrix} \vvec{q}	\\
		&=&	\phi^\dagger \sigma_i \chi	\\
	\vbar(q) \sigma^{0i} u(p)
		&=&	i \vdaggervec{q} \begin{pmatrix} 0 & \sigma_i \\ -\sigma_i & 0  \end{pmatrix} \uvec{p}	\\
		&=& - i \chi^\dagger \sigma_i \phi	\\
\eeqa
Then the part with two spatial indices:
\beqa
	\ubar(p') \sigma^{ij} v(q)
		&=& 	\epsilon_{ijk}\udaggervec{p'} \begin{pmatrix} \sigma_k & 0 \\ 0 & \sigma_k  \end{pmatrix} \vvec{q}	\\
		&=&	\epsilon_{ijk} \phi^\dagger \left( \frac{ \sigma_k \sigdot{q} + \sigdot{p'}\sigma_k }{2m} \right ) \chi	\\
	\vbar(q) \sigma^{ij} u(p)
		&=&	\epsilon_{ijk}\vdaggervec{q} \begin{pmatrix} \sigma_k & 0 \\ 0 & \sigma_k  \end{pmatrix}  \uvec{p}	\\
		&=&	\epsilon_{ijk} \chi^\dagger \left( \frac{ \sigma_k \sigdot{p} + \sigdot{q}\sigma_k }{2m} \right) \phi
\eeqa
These terms will not contribute to the order desired.  So
\beqa
i \frac{q_\nu}{2m}\ubar(p') \sigma^{0\nu} v(q)
	&= &	i \frac{q_j}{2m}\ubar(p') \sigma^{0j} v(q) \\
	&\approx&	i \frac{q_j}{2m} \phi^\dagger \sigma_i \xi	
\eeqa

\beqa
i \frac{q_\nu}{2m}\ubar(p') \sigma^{i\nu} v(q)
	&= &	i \frac{q_j}{2m}\ubar(p') \sigma^{ij} v(q) \\
	&\approx&	i \frac{q_j}{2m} 	
\eeqa
...
Looking at right hand sides, there are two types of terms to calculate:
...
\left( 1 - \frac{k_0}{2m} \right ) \frac{1}{2m} 
	...
	\left[ \epsilon_j(k) \chi^\dagger \left( F_1 - F_2 \frac{k_0}{2m} \right)  \gv{\sigma} \cdot  \gv{\epsilon}(k)   \phi \right ]	
$
} 
\vspace{1em}
...
%TODO add diagram?
\beqa
(\text{Sum of Z diagrams}) &=& \frac{1}{2m} \phi^\dagger \Bigg [
	 \left( F_1^2 + F_1 [F_1 + 2 F_2 ] \frac{k_0}{2m} \right )
			\gv{\sigma} \cdot \gv{\epsilon}^(k)  \gv{\sigma} \cdot  \gv{\epsilon}^*(k')  \\
	&&+ 
			\left( F_1^2 - F_1 [F_1 + 2 F_2 ] \frac{k_0}{2m} \right )
